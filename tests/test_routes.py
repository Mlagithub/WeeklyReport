...
class TestAuthentication:
    ...
    def test_login_success(self, client, test_user):
        ...
        response = client.post(
            "/login", data={"username": test_user["username"], "password": test_user["password"]}, follow_redirects=True
        )
        assert response.status_code == 200
    ...
    def test_login_invalid_password(self, client, test_user):
        ...
        response = client.post(
            "/login", data={"username": test_user["username"], "password": "wrongpassword123"}, follow_redirects=True
        )
        assert response.status_code == 200
        ...
        assert (
            ...
            or b"\xe4\xb8\x8d\xe6\xad\xa3\xe7\xa1\xae" in response.data
            or b"is-invalid" in response.data
        )

    def test_login_nonexistent_user(self, client):
        """Test login with a username that doesn't exist."""
        response = client.post(
            "/login", data={"username": "nonexistentuser", "password": "somepassword123"}, follow_redirects=True
        )
        assert response.status_code == 200
        # Should stay on login page (form re-rendered with error)
        # Check for flash message or form error indicator
    ...
    def test_register_password_mismatch(self, client):
        ...
        response = client.post(
            "/register",
            data={"username": "mismatchuser", "password": "Password123", "password_confirm": "DifferentPassword123"},
            follow_redirects=True,
        )
        assert response.status_code == 200
...
def create_user_helper(client, username, password, roles=None):
    ...
    with client.application.app_context():
        user = user_datastore.create_user(
            email=f"{username}@test.com", username=username, password=hash_password(password), roles=roles or []
        )
        db.session.commit()
...
class TestRecordCRUD:
    ...
    def test_create_record_success(self, auth_client):
        ...
        response = auth_client.post(
            "/create_records", data={"date": "2026-03-23", "body": "<p>Test content</p>"}, follow_redirects=True
        )
        assert response.status_code == 200
    ...
    def test_manage_records_shows_user_records(self, auth_client, test_user):
        ...
        auth_client.post(
            "/create_records", data={"date": "2026-03-23", "body": "<p>User record content</p>"}, follow_redirects=True
        )
    ...
    def test_edit_record_page_loads(self, auth_client):
        ...
        # First create a record
        auth_client.post(
            "/create_records", data={"date": "2026-03-23", "body": "<p>Original content</p>"}, follow_redirects=True
        )
    ...
    def test_edit_record_success(self, auth_client):
        ...
        # Then edit it
        response = auth_client.post(
            "/edit_record/1", data={"date": "2026-03-24", "body": "<p>Updated content</p>"}, follow_redirects=True
        )
        assert response.status_code == 200
    ...
    def test_edit_record_forbidden(self, client, test_user):
        ...
        client.post(
            "/create_records", data={"date": "2026-03-23", "body": "<p>User A record</p>"}, follow_redirects=True
        )
        ...
        client.post("/login", data={"username": "userb", "password": "UserBPass123"})
    ...
    def test_delete_record_success(self, auth_client):
        ...
        # First create a record
        auth_client.post(
            "/create_records", data={"date": "2026-03-23", "body": "<p>Record to delete</p>"}, follow_redirects=True
        )
        # Then delete it
    ...
    def test_delete_record_forbidden(self, client, test_user):
        ...
        client.post(
            "/create_records",
            data={"date": "2026-03-23", "body": "<p>User A record to protect</p>"},
            follow_redirects=True,
        )
        client.get("/logout", follow_redirects=True)
        ...
        client.post("/login", data={"username": "userc", "password": "UserCPass123"})
    ...
    def test_admin_can_edit_any_record(self, client, test_user):
        ...
        client.post(
            "/create_records",
            data={"date": "2026-03-23", "body": "<p>User record for admin test</p>"},
            follow_redirects=True,
        )
        client.get("/logout", follow_redirects=True)
...
class TestHomeRendering:
    ...
    def test_home_shows_bold_text(self, auth_client):
        ...
        auth_client.post(
            "/create_records",
            data={"date": "2026-03-26", "body": "<p>This is <b>bold</b> text</p>"},
            follow_redirects=True,
        )
    ...
    def test_home_shows_italic_text(self, auth_client):
        ...
        auth_client.post(
            "/create_records",
            data={"date": "2026-03-26", "body": "<p>This is <i>italic</i> text</p>"},
            follow_redirects=True,
        )
    ...
    def test_home_shows_list_items(self, auth_client):
        ...
        auth_client.post(
            "/create_records",
            data={"date": "2026-03-26", "body": "<ul><li>Item 1</li><li>Item 2</li></ul>"},
            follow_redirects=True,
        )
...
class TestXSSPrevention:
    ...
    def test_xss_script_filtered_on_home(self, auth_client):
        ...
        auth_client.post(
            "/create_records",
            data={"date": "2026-03-26", "body": '<script>alert("xss")</script><p>safe content</p>'},
            follow_redirects=True,
        )
    ...
    def test_xss_onclick_filtered_on_home(self, auth_client):
        ...
        auth_client.post(
            "/create_records",
            data={"date": "2026-03-26", "body": '<a onclick="evil()">click me</a>'},
            follow_redirects=True,
        )
    ...
    def test_xss_javascript_url_filtered_on_home(self, auth_client):
        ...
        auth_client.post(
            "/create_records",
            data={"date": "2026-03-26", "body": '<a href="javascript:alert(1)">dangerous link</a>'},
            follow_redirects=True,
        )
    ...
    def test_xss_onerror_filtered_on_home(self, auth_client):
        ...
        auth_client.post(
            "/create_records",
            data={"date": "2026-03-26", "body": '<img src="x" onerror="alert(1)">'},
            follow_redirects=True,
        )
